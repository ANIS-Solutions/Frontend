...
export default function AppsList({ childId }: AppsListProps) {
  ...
  const filtered = apps.filter((app) => {
    ...
    if (filter === "limited") return app.settings.dailyLimit > 0 && !app.settings.isBlocked;
    if (filter === "active") return !app.settings.isBlocked && app.settings.dailyLimit === 0;
    return true;
  });
  ...
  const stats = {
    ...
    limited: apps.filter((a) => a.settings.dailyLimit > 0 && !a.settings.isBlocked).length,
  };
  ...
  if (error) {
    return (
      <div className="p-4 rounded-xl text-sm" style={{ background: "#FCEBEB", color: "#F72B35" }}>
        {error}
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        ...
        >
          <Monitor size={32} className="mx-auto mb-2" style={{ color: "#d1d5db" }} />
          <p className="text-sm font-medium mb-1" style={{ color: "#9ca3af" }}>
            {apps.length === 0 ? "No apps installed yet" : "No apps match this filter"}
          </p>
