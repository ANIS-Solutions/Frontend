...
function getAppName(
  app: import("@/app/types/api/usage.types").AppUsage,
): string {
...
export default function WeeklyChart({ weekUsage }: WeeklyChartProps) {
  ...
  const maxMinutes = Math.max(
    ...weekUsage.map((d) => d.totalScreenTimeMinutes),
    1,
  );
  ...
  return (
    <div className="rounded-2xl p-5 border-[0.5px] bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
      ...
      {/* Chart */}
      <div
        className="flex items-end gap-1.5 sm:gap-3 px-1 justify-start"
        style={{ height: 140 }}
      >
        {sorted.map((day) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-1.5 group relative flex-1 sm:flex-none sm:w-12 min-w-0"
              style={{
                background: "none",
                border: "none",
                cursor: "pointer",
        ...
        {effectiveSelected.apps.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {[...effectiveSelected.apps]
              ...
              .map((app, i) => {
                const maxAppTime = Math.max(
                  ...effectiveSelected.apps.map((a) => a.totalAppTimeMinutes),
                );
                const widthPercent =
                  maxAppTime > 0
                    ? (app.totalAppTimeMinutes / maxAppTime) * 100
                    : 0;
                const colors = [
                  "#1E73BE",
                  "#854F0B",
                ...
                return (
                  <div key={`${app.packageName}-${i}`}>
                    <div className="flex items-center gap-2.5 mb-1">
                      {/*  Icon */}
                      <div className="w-7 h-7 rounded-md flex items-center justify-center shrink-0 overflow-hidden bg-gray-100 dark:bg-gray-700">
                        {app.iconUrl ? (
                      ...
                      <p className="text-xs font-medium truncate text-gray-700 dark:text-gray-300 flex-1 min-w-0">
                        {getAppName(app)}
                      </p>
                      <span
                    ...
                    <div className="rounded-full h-1.5 bg-gray-100 dark:bg-gray-700 ml-8.5">
                      <div
